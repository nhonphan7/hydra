...
class Trainer:
    ...
    def __init__(self, 
        ...
        if self.scale:
            ...
            self.X_scale_ = torch.tensor(X_scaler.scale_, dtype=torch.float).to(self.device)
            self.X_min_ = torch.tensor(X_scaler.min_, dtype=torch.float).to(self.device)
            y_scaler = load('scales/y_scaler.pkl')
            self.y_scale_ = torch.tensor(y_scaler.scale_, dtype=torch.float).to(self.device)
            self.y_min_ = torch.tensor(y_scaler.min_, dtype=torch.float).to(self.device)
        if self.func_of == 'D' and not self.energy:
            self.X_sf = torch.tensor([[0., 0., 0., 0., 0., 0.]], requires_grad=True).to(self.device)
            if self.scale:
    ...
    def train_step(self, batch: torch.Tensor, model: nn.Module, 
        ...
        if self.func_of == 'D' and not self.energy:
            features = torch.cat((features, self.X_sf))
        ...
        if self.sobolev:
            predictions_grad = grad(predictions.sum(), features, create_graph=True)[0]
            if self.func_of == 'D' and self.scale:
                if self.energy:
                    S_unscale = predictions_grad * self.X_scale_ / self.y_scale_[0]
                    S_scale = S_unscale * self.y_scale_[-6:] + self.y_min_[-6:]
                    loss = F.mse_loss(predictions, targets[:, 0])
                    loss += F.mse_loss(S_scale, targets[:, -6:])
                else:
                    S_unscale = predictions_grad[:-1] * self.X_scale_
                    S_scale = S_unscale * self.y_scale_ + self.y_min_
                    loss = F.mse_loss(predictions[-1].unsqueeze(0), torch.zeros(1).to(self.device))
                    loss += F.mse_loss(S_scale, targets)
                self.update_metric(metric, S_scale, targets[:, -6:], weights)
        else:
    ...
    def train(self, model, train_dl, val_dl, optimizer, scheduler, writer, checkpointer, metric):
        ...
        with tqdm(range(num_epochs), disable=self.disable_tqdm) as pbar_epoch:
            for epoch in pbar_epoch:
                ...
                desc = (f"\nEpoch({epoch}):"
                    f"\n\tTraining Loss: {loss_train.detach().cpu().numpy().item():.10f}"
                    f"\n\tValidation Loss: {loss_val.detach().cpu().numpy().item():.10f}")
                if metric:    
                    desc += f'\n\t{self.metric_name}: {metric_train:.10f}'
