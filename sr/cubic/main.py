...
if __name__ == '__main__':
    ...
    noise_factor = 0.
    if noise_factor > 0:
        data_dir = f'../../nam/cubic/cubic_D9_noise{int(noise_factor * 100)}.csv'
    else:
        data_dir = '../../nam/cubic/cubic_D9.csv'
        # data_dir = '../../nam/cubic/cubic_D12.csv'
    plot_cases = [0, 6]
    strain_comp = 11
    ...
    g = df[['g1', 'g2', 'g3', 'g4', 'g5', 'g6', 'g7', 'g8', 'g9']].to_numpy()
    gp = df[[
        'gp1', 'gp2', 'gp3', 'gp4', 'gp5', 'gp6', 'gp7', 'gp8', 'gp9'
    ]].to_numpy()
    if noise_factor > 0:
        lin_weight = np.load(
            f'../../nam/cubic/lin_weight_D9_noise{int(noise_factor * 100)}.npy'
        )
    else:
        lin_weight = np.load('../../nam/cubic/lin_weight_D9.npy')
    # g = df[[
    ...
    #     'gp12'
    # ]].to_numpy()
    # lin_weight = np.load('../../nam/cubic/lin_weight_D12.npy')
    if scale:
    ...
    bias = (energy_nam * y_scaler.scale_[0] + y_scaler.min_[0]) - g.sum(axis=1)
    # energy_eq = construct_eq(func_of, voigt, lin_weight, g_eq, bias[0])
    # print(energy_eq)

    energy_sr_train_plot = (
        g_sr_train_plot.sum(axis=1) + bias[0] - y_scaler.min_[0]
    ) / y_scaler.scale_[0]
    energy_sr_test_plot = (
        g_sr_test_plot.sum(axis=1) + bias[0] - y_scaler.min_[0]
    ) / y_scaler.scale_[0]
    stress_sr_train_plot = (
        gp_sr_train_plot @ lin_weight * X_scaler.scale_ / y_scaler.scale_[0]
    )
    stress_sr_test_plot = (
        gp_sr_test_plot @ lin_weight * X_scaler.scale_ / y_scaler.scale_[0]
    )
    plot_DPsi(
        func_of,
        X,
