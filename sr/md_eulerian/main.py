...
if __name__ == '__main__':
    ...
    # case 2: uniaxial compression (33 direction)
    # case 3: positive simple shear (12 direction)
    # case 4: positive simple shear (13 direction)
    # case 5: positive simple shear (23 direction)
    # case 6: negative simple shear (13 direction)
    # case 7: negative simple shear (23 direction)
    # case 8: negative simple shear (12 direction)
    # case 9: uniaxial tension (11 direction)
    # case 10: uniaxial tension (22 direction)
    # case 11: uniaxial tension (33 direction)
    # case 12: equibiaxial compression (11 and 22 directions)
    # case 13: equibiaxial compression (22 and 33 directions)
    # case 14: equibiaxial compression (11 and 33 directions)

    data_dir = '../../nam/md_eulerian/all_Data_predicted_D18.csv'
    plot_cases = [0, 9]
    strain_comp = 11
    ...
    X[:, -3:] *= 2.
    y = df[['sd11', 'sd22', 'sd33', 'sd23', 'sd13', 'sd12']].to_numpy()
    energy_nam = df['psi_nam'].to_numpy()
    stress_nam = df[[
        'sd11_nam', 'sd22_nam', 'sd33_nam', 'sd23_nam', 'sd13_nam', 'sd12_nam'
    ]].to_numpy()
    g = df[[
        'g1',
        'g2',
    ...
    lin_weight = np.load('../../nam/md_eulerian/lin_weight_D18.npy')
    if scale:
    ...
    stress_nam_train = stress_nam[train_idx]
    stress_nam_test = stress_nam[test_idx]
    nam_inputs_train = nam_inputs[train_idx]
    nam_inputs_test = nam_inputs[test_idx]
    g_nam_train = g[train_idx]
    g_nam_test = g[test_idx]
    gp_nam_train = gp[train_idx]
    gp_nam_test = gp[test_idx]
    ...
    stress_nam_train_plot = stress_nam[train_plot_idx]
    stress_nam_test_plot = stress_nam[test_plot_idx]
    ...
    gp_nam_train_plot = gp[train_plot_idx]
    gp_nam_test_plot = gp[test_plot_idx]
    ...
    for k in range(nam_inputs.shape[1]):
        model.fit(
            ...
            weights=gp_nam_train[::fit_step, k],
            variable_names=['x']
        )
        ...
        grad_diff = gp_sr_train[:, k] - gp_nam_train[:, k]
    ...
    stress_sr_train_plot = gp_sr_train_plot @ lin_weight * X_scaler.scale_
    stress_sr_test_plot = gp_sr_test_plot @ lin_weight * X_scaler.scale_
    for comp in voigt:
        plot_DSDcomp(
            X,
            X_train_plot,
            ...
            strain_comp,
            y,
            stress_nam_train_plot,
            stress_nam_test_plot,
            stress_sr_train_plot,
            stress_sr_test_plot,
            comp,
            train_plot_range,
            test_plot_range,
